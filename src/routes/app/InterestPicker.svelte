<script>
	...
	let { open = $bindable(false), categories = $bindable([]), allCategories, onchange } = $props();
	...
	let languages = [...new Set(allCategories.map((p) => p.categories.map((c) => c.lang)).flat())];
	...
	$effect(() => {
		...
		if (!activeProviderButton || !open) return;
...
<Modal bind:open color={COLOR} containerClasses="max-md:h-full">
	...
	<!-- Provider selector -->
	<HorizontalScroll class="gap-4 w-full px-4 shrink-0 mb-4 relative" id="activeProviderContainer">
		<span
	...
	<!-- Categories list -->
	<div class="grow flex overflow-y-hidden w-full flex-col md:h-[70vh] p-4 pt-0">
		<div class="overflow-y-auto no-scrollbar grow rounded-2xl flex flex-col relative">
